...
class MultiInputTransformerWrapper(Module):
    def __init__(
        ...
        *,
        ...
        self.embeds = ModuleDict(
            {
                f"{name}_embed": nn.Embedding(one_num_tokens, emb_dim)
        ...
        if return_only_embed:
            ...
            self.to_logits = ModuleDict(
                {
                    name: nn.Linear(dim, logits_dim, bias=False)
    ...
    def forward(
        ...
        **kwargs,
        ...
        pos_emb = (
            self.pos_emb(first_input, pos=pos, seq_start_pos=seq_start_pos)
