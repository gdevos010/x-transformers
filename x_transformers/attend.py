...
def selective_attn(sim, sim_head_gate=None, no_mask_sos=True):
    ...
    if j > i:
        eye = F.pad(eye, (j - i, 0), value=1.0)

    gate = (1.0 - eye) * gate
    gate = F.pad(gate, (0, 0, 1, -1), value=0.0)  # only allow for masking the future
...
def one_hot_straight_through(logits, temperature=1.0):
    one_hot_indices = logits.argmax(dim=-1, keepdim=True)
    one_hot = torch.zeros_like(logits).scatter(-1, one_hot_indices, 1.0)
...
def sparse_topk_attn(logits, sparse_topk, temperature=1.0, straight_through=False):
    orig_logits = logits
...
class Attend(Module):
    def __init__(
        ...
        *,
        ...
        assert not (flash and is_sparse_topk_attn), (
            "topk attention not available for flash"
        )
        ...
        if exists(custom_attn_fn):
            ...
            self.attn_fn = (
                partial(softmax_fn, dtype=torch.float32) if not qk_norm else softmax_fn
            )
        ...
        assert not (
            flash
            and (
                pre_talking_heads or post_talking_heads or pre_scale_post_talking_heads
            )
        ), "talking heads not compatible with flash attention"

        self.pre_softmax_talking_heads = (
            nn.Conv2d(heads, heads, 1, bias=False) if pre_talking_heads else None
        )
        self.post_softmax_talking_heads = (
            nn.Conv2d(heads, heads, 1, bias=False) if post_talking_heads else None
        )
        self.pre_scale_post_talking_heads = (
            nn.Conv2d(heads, heads, 1, bias=False)
            if pre_scale_post_talking_heads
            else None
        )
        ...
        assert not (flash and selective), (
            "selective attention cannot work on flash attention"
        )
        assert not (selective and not causal), (
            "selective attention is designed for autoregressive"
        )
        ...
        if softclamp_logits:
            assert not flash, (
                "flash attention not compatible with logit softclamp value yet"
            )
            assert logit_softclamp_value > 0.0
        ...
        assert not (flash and torch_version < version.parse("2.0.0")), (
        ...
        if self.flash:
            if torch_version >= version.parse("2.3"):
                ...
                sdpa_backends = [
                    str_to_backend[enable_str]
                    for enable_str, enable in sdp_kwargs.items()
                    if enable
                ]

                self.sdp_context_manager = partial(
                    torch.nn.attention.sdpa_kernel, sdpa_backends
                )
            else:
                self.sdp_context_manager = partial(
                    torch.backends.cuda.sdp_kernel, **sdp_kwargs
                )
    ...
    def forward(self, q, k, v, mask=None, attn_bias=None, prev_attn=None):
        ...
        if kv_heads == 1:
            ...
            k, v = tuple(
                repeat(t, "b kvh n d -> b (r kvh) n d", r=heads // kv_heads)
                for t in (k, v)
            )
        ...
        if self.add_zero_kv:
            k, v = tuple(F.pad(t, (0, 0, 1, 0), value=0.0) for t in (k, v))
