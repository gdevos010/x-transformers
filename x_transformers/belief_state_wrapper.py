...
class BeliefStateWrapper(Module):
    ...
    def __init__(
        ...
        train_frac_forward_backward_pairs: float = 1.0,
        text_head: Module | None = None,
        backward_ar_loss_weight: float = 1.0,  # can weigh the training of the backwards decoder differently, perhaps fwd/bwd have a shared backbone etc etc
        pred_distance=False,
        pred_distance_loss_weight: float = 1.0,
        cond_on_distance=False,
        cond_on_distance_prob=0.5,
        max_pred_distance=None,
    ):
        super().__init__()
        backward_decoder = default(
            backward_decoder, forward_decoder
        )  # if backward decoder not set, use the same transformer, assume it knows how to switch gears based on suffix token

        assert forward_decoder.emb_dim == backward_decoder.emb_dim, (
            "forward and backwards model must have the same embedding dimension"
        )
        assert forward_decoder.num_tokens == backward_decoder.num_tokens, (
            "forward and backwards model must have the same number of tokens"
        )
        ...
        assert 0.0 < cond_on_distance_prob < 1.0
        ...
        assert 0 < train_frac_forward_backward_pairs <= 1.0
        self.train_frac_fb_pairs = train_frac_forward_backward_pairs
        self.needs_subsample_fb_pairs = train_frac_forward_backward_pairs < 1.0

        # loss weighting

        self.backward_ar_loss_weight = backward_ar_loss_weight
        self.needs_loss_weight = backward_ar_loss_weight != 1.0

        self.register_buffer(
            "loss_weights", tensor([1.0, self.backward_ar_loss_weight])
        )
        ...
        **kwargs,
        ...
        if not decode_backwards:
            ...
            suffix_embed = self.backward_decoder(
                suffix, prepend_embeds=suffix_sos_tokens, return_embeddings=True
            )
    ...
    def forward(
        ...
        backward_embeds = self.backward_decoder(
            backward_seq, prepend_embeds=suffix_tokens, return_embeddings=True
        )
        ...
        forward_embeds, backward_embeds = detach_multiple(
            forward_embeds, backward_embeds
        )
        ...
        fb_pairs = cartesian_prod(
            seq_arange, seq_arange + 1
        )  # plus one for suffix token
        ...
        if self.needs_subsample_fb_pairs:
            ...
            rand_subsampled_indices = torch.randperm(num_pairs, device=device)[
                :num_subsampled
            ]
        ...
        forward_labels, backward_labels = (
            seq_for_labels[:, labels_fi],
            seq_for_labels[:, labels_bi],
        )
        ...
        fb_embeds = cat((forward_embeds[:, fi], backward_embeds[:, bi]), dim=-1)
        ...
        cond_on_distance = self.cond_on_distance and (
            random() < self.cond_on_distance_prob
        )
        ...
        if exists(self.to_distance_logits) and not cond_on_distance:
            ...
            pred_dist_loss = F.cross_entropy(
                rearrange(distance_logits, "b n l -> b l n"), distance_labels
            )

            loss = loss + pred_dist_loss * self.pred_distance_loss_weight

        # maybe loss weighting

        needs_loss_weight = default(
            self.needs_loss_weight, exists(loss_weight_by_fb_indices)
        )
