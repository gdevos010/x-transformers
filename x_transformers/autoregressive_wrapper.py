...
FILTER_LOGITS_FN = dict(top_p=top_p, top_k=top_k, top_a=top_a, min_p=min_p)
...
class AutoregressiveWrapper(Module):
        ...
        **kwargs,
        ...
        for i in range(seq_len):
            ...
            if should_cache and is_first:
                modify_cached_kv(
                    cache, lambda t: repeat(t, "b ... -> (b beams) ...", beams=beams)
                )
            ...
            scores = rearrange(
                scores,
                "(b prev_beams) next_beams -> b (prev_beams next_beams)",
                b=batch,
            )
            ...
            if curr_num_beams > beams:
                ...
                top_beams_indices = (
                    curr_num_beams * batch_arange[:, None] + top_beams_indices
                )

                flattened_beam_indices = rearrange(
                    top_beams_indices, "b beams -> (b beams)"
                )
        ...
        (out,) = unpack(
            out, packed_shape, "* beams n"
        )  # prompt may have no batch dimension
    ...
    def generate(
        ...
        **kwargs,
        ...
        max_seq_len, greedy = self.max_seq_len, temperature == 0.0

        # handle prompts given as list of variable lengthed token ids

        if isinstance(prompts, list):
            assert len(prompts) > 0, "prompts cannot be empty list"
            assert not exists(prompt_lens), (
                "`prompt_len` will be auto derived if prompts are passed in as list of Tensors"
            )

            prompt_lens = tensor(
                [t.shape[0] for t in prompts], device=prompts[0].device
            )
    ...
    def forward(self, x, return_outputs=False, prepend_embeds=None, **kwargs):
        seq, ignore_index, add_attn_z_loss, add_next_embed_loss = (
            x.shape[1],
            self.ignore_index,
            self.add_attn_z_loss,
            self.add_continuous_pred_head,
        )
