...
def sample_from_mean_variance(mean, variance, eps=1e-5, temperature=1.0):
...
def masked_mean(t, mask):
    ...
    masked_average = num / den.clamp(min=1.0)
...
class ContinuousTransformerWrapper(Module):
    def __init__(
        ...
        *,
        ...
        self.project_in = (
            nn.Linear(dim_in, dim, bias=False) if exists(dim_in) else nn.Identity()
        )
        ...
        self.project_out = (
            nn.Linear(dim, dim_out * (2 if probabilistic else 1), bias=False)
            if exists(dim_out)
            else nn.Identity()
        )
        ...
        **kwargs,
        ...
        if exists(prepend_embeds):
            ...
            if exists(prepend_mask) or exists(mask):
                mask = default(
                    mask,
                    lambda: torch.ones((batch, seq), device=device, dtype=torch.bool),
                )
                prepend_mask = default(
                    prepend_mask,
                    lambda: torch.ones(
                        (batch, prepend_seq), device=device, dtype=torch.bool
                    ),
                )
        ...
        if not return_embeddings and self.probabilistic:
            mean, log_var = rearrange(
                out, "... (d mean_log_var) -> mean_log_var ... d", mean_log_var=2
            )
...
class ContinuousAutoregressiveWrapper(Module):
    def __init__(
        ...
        loss_fn = default(
            loss_fn,
            nn.MSELoss(reduction="none") if not probabilistic else GaussianNLL(),
        )
    ...
    @torch.no_grad()
    def generate(self, start_tokens, seq_len, temperature=1.0, cache_kv=True, **kwargs):
        ...
        for _ in range(seq_len):
            ...
            net_out, new_cache = self.net(
                x, cache=cache, return_intermediates=True, **kwargs
            )
            ...
            if self.probabilistic:
                ...
                last_output = sample_from_mean_variance(
                    mean, var, temperature=temperature
                )
    ...
    def forward_rollout(self, x, rollout_steps=2, **kwargs):
        ...
        if exists(lens):
            assert "mask" not in kwargs, (
                "either `mask` or `lens` passed in, but not both"
            )
        ...
        batch = x.shape[0]
        seq_start_pos = (
            (torch.rand((batch,), device=device) * valid_tokens_for_rollout)
            .floor()
            .long()
        )
        ...
        for _ in range(steps):
            out, cache = self.net(
                inp, seq_start_pos=seq_start_pos, return_intermediates=True, **kwargs
            )
