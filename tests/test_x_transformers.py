...
def test_kv_cache():
    model = TransformerWrapper(
        num_tokens=20000,
        max_seq_len=1024,
        attn_layers=Decoder(dim=8, depth=2, heads=4, cross_attend=True),
    )
    ...
    logits, cache = model(prompts, context=context, return_intermediates=True)
...
def test_cope():
    model = TransformerWrapper(
        num_tokens=256,
        max_seq_len=1024,
        attn_layers=Decoder(dim=8, depth=2, heads=4, attn_use_cope=True),
    )
...
def test_unet_skip(depth):
    model = TransformerWrapper(
        num_tokens=20000,
        max_seq_len=1024,
        attn_layers=Encoder(dim=128, depth=depth, heads=8, unet_skips=True),
    )
...
def test_reinject_input():
    model = TransformerWrapper(
        ...
        max_seq_len=1024,
        recycling=True,
        attn_layers=Decoder(dim=512, depth=12, heads=8, reinject_input=True),
    )
...
def test_value_residual(learned_value_residual_mix: bool):
    model = TransformerWrapper(
        num_tokens=20000,
        max_seq_len=1024,
        attn_layers=Decoder(
...
def test_forgetting_transformer(has_num_mem_kv: bool):
    model = TransformerWrapper(
        num_tokens=20000,
        max_seq_len=1024,
        attn_layers=Decoder(
...
def test_custom_alibi(flash: bool):
    model = TransformerWrapper(
        ...
        max_seq_len=1024,
        attn_layers=Decoder(
            dim=512, depth=2, heads=8, alibi_pos_bias=True, attn_flash=flash
        ),
    )
...
def test_custom_rotary_pos_emb(rotary_xpos):
    ...
    model = TransformerWrapper(
        ...
        max_seq_len=1024,
        attn_layers=Decoder(
            dim=512, depth=2, heads=8, rotary_pos_emb=True, rotary_xpos=rotary_xpos
        ),
    )
...
def test_custom_alibi_across_heads(flash: bool):
    model = Decoder(
        ...
        heads=2,
        alibi_pos_bias=True,
        rel_pos_kwargs=dict(slopes=[1, 1]),
        attn_flash=flash,
    )

    x = torch.randn(2, 4, 512)

    pos = torch.tensor([[[0, 1, 2, 4], [1, 3, 5, 7]], [[2, 3, 4, 5], [6, 8, 9, 10]]])
...
def test_laser():
    model = TransformerWrapper(
        num_tokens=20000,
        max_seq_len=1024,
        attn_layers=Decoder(dim=128, depth=6, heads=8, attn_laser=True),
    )
...
def test_cross_attn_rotary(self_attn_custom_pos: bool, cross_attn_rotary: bool):
...
def test_hyper_connections(tanh):
    model = TransformerWrapper(
        ...
        attn_layers=Decoder(
            ...
            residual_fn_kwargs=dict(tanh=tanh),
        ),
    )
...
def test_caching_when_inputs_not_include_past():
    ...
    out2, cache = model(
        x2, cache=cache, return_intermediates=True, input_not_include_cache=True
    )
    out3, cache = model(
        x3, cache=cache, return_intermediates=True, input_not_include_cache=True
    )
...
def test_lime(num_residual_streams, integrate_layers):
    model = TransformerWrapper(
        num_tokens=20000,
        max_seq_len=1024,
        attn_layers=Decoder(
...
@pytest.mark.parametrize("backward_ar_loss_weight", (1.0, 0.5))
...
def test_belief_state_wrapper(
    backward_ar_loss_weight, goal_suffix, pred_distance, variable_len
):
    ...
    forward_model = TransformerWrapper(
        num_tokens=20000,
        max_seq_len=1024,
        attn_layers=Decoder(dim=512, depth=6, heads=8, rotary_pos_emb=True),
    )
...
def test_entropy_based_tokenizer(var_length):
...
def test_entropy_based_tokenizer_max_token_len():
    ...
    tokenizer = EntropyBasedTokenizer(model, entropy_threshold=100, max_token_size=4)
...
def test_autoregressive_wrapper(add_continuous_pred_head):
...
def test_up(keep_buffer_on_cpu):
    ...
    up_wrapper = UniversalPretrainWrapper(
        model, seq_len=16, keep_buffer_on_cpu=keep_buffer_on_cpu
    )
...
def test_beam_search(stochastic):
    ...
    beams, scores = wrapper.beam_search(
        x[:, :1], 10, beams=4, return_beams_and_scores=True, stochastic=stochastic
    )
...
def test_attn_pooler(num_pooled_tokens, attn_pool_depth):
    model = TransformerWrapper(
        num_tokens=256,
        max_seq_len=1024,
        attn_pool=True,
        ...
        attn_layers=Encoder(dim=512, depth=12, heads=8, attn_value_rmsnorm=True),
    )
