...
model = BeliefStateWrapper(
    forward_decoder=forward_model, backward_decoder=backward_model
)
...
for i in tqdm.tqdm(range(NUM_BATCHES), mininterval=10.0, desc="training"):
    ...
    if i % GENERATE_EVERY == 0:
        ...
        sample = model.generate_with_suffix_cond(
            prompts=inp, seq_len=GENERATE_LENGTH, cache_kv=True
        )
        ...
        sample = model.generate_with_suffix_cond(
            prompts=inp, seq_len=GENERATE_LENGTH, cache_kv=True, decode_backwards=True
        )
